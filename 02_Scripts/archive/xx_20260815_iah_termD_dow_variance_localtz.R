...
raw <- dbGetQuery(con, "
  SELECT time, wait_time
  FROM tsa_wait_times
  WHERE airport = 'IAH' AND checkpoint = 'IAH Terminal D'
") |>
  as_tibble() |>
  mutate(time = force_tz(time, tzone = "UTC"),
         time_local = with_tz(time, tzone = "America/Chicago")) |>
  filter(time_local >= max(time_local, na.rm = TRUE) - ddays(42))
...
profile <- raw |>
  ...
  summarise(
    ...
    sd_wait = round(sd(wait_time, na.rm = TRUE), 2),
    min_wait = min(wait_time, na.rm = TRUE),
    max_wait = max(wait_time, na.rm = TRUE),
    pct_at_floor = round(100 * mean(wait_time == 5, na.rm = TRUE), 1),
    .groups = "drop"
  ) |>
  arrange(dow, hr)
