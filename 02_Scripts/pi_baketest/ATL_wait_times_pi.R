# 02_Scripts/pi_baketest/ATL_wait_times_pi.R
#
# Phase 0 bake-off variant of ../ATL_wait_times.R. Scrape/transform logic is
# unchanged from the production script. Unlike JFK/LGA/EWR, the production
# ATL script never calls chromote::local_chrome_version() itself, so there is
# no discovery call to drop here -- options(chromote.chrome = <path to system
# chromium-browser>), set by the orchestrator before sourcing this file, is
# sufficient on its own. The only functional change is writing to the scratch
# DB (con_write) instead of the production connection.
#
# Keep in sync with ../ATL_wait_times.R if the scrape/transform logic there
# changes.
